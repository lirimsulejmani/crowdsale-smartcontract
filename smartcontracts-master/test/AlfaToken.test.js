...
contract('AlfaToken (Basic Tests)', function (accounts) {
...
contract('AlfaToken (transfers)', function (accounts) {
    const owner = accounts[0];
    const user1 = accounts[1];
...
contract('AlfaToken (token burning tests)', function (accounts) {
    ...
    beforeEach(async function () {
        token = await AlfaToken.new(owner);
    });
