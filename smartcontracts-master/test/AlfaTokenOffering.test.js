...
contract('AlfaTokenOffering constructor', function (accounts) {
    ...
    it('Constructor failure cases', async function () {
        const token = await AlfaToken.new(admin, { from: owner });
        // etherToAlfa rate is 0
        await util.assertRevert(AlfaTokenOffering.new(
...
contract('Offering stage changes correctly', function (accounts) {
    ...
    it('Start and end offering correctly', async function () {
        const token = await AlfaToken.new(admin, { from: owner });
        const offering = await AlfaTokenOffering.new(
...
contract('Whitelist Crowdsale', function (accounts) {
    ...
    beforeEach(async function () {
        token = await AlfaToken.new(admin, { from: owner });
        offering = await AlfaTokenOffering.new(
            5000, beneficiary, util.toEther(1), token.address, { from: owner }
        );
