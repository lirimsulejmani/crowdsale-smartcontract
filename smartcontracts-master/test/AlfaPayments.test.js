...
contract('AlfaPayments Dispatch Test', function (accounts) {
    ...
    beforeEach(async function () {
        payments = await AlfaPayments.new(user1, aFee, { from: owner});
        token = await AlfaToken.new(user1, { from: owner});
    });
...
contract('AlfaPayments Cancel Test', function (accounts) {
    const owner = accounts[0];
    const user1 = accounts[1];
    const demand = accounts[2];
    const supply = accounts[3];
...
contract('AlfaPayments Arbitration Host Test', function (accounts) {
    const owner = accounts[0];
    const user1 = accounts[1];
    const demand = accounts[2];
    const supply = accounts[3];
    ...
    beforeEach(async function () {
        payments = await AlfaPayments.new(arbiterAddress, aFee, { from: owner});
        token = await AlfaToken.new(user1, { from: owner});
    });
