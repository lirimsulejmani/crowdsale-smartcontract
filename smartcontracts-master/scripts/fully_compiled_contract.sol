...
/*
 * AlfaToken is a standard ERC20 token with some additional functionalities:
...
contract AlfaToken is StandardToken, BurnableToken, Ownable {
    string public constant symbol = "α";
    string public constant name = "Alfa Token";
     ...
     */
    function AlfaToken(address admin) public {
     ...
     * @param value Amount of Alfatokens to send
     */
...
contract AlfaTokenOffering is Pausable {
    using SafeMath for uint256;
     ...
     *  
     * @param AlfaToEtherRate Number of Alfatokens per ether
     ...
     */
    function AlfaTokenOffering(
        uint256 alfaToEtherRate, 
        address beneficiaryAddr, 
        uint256 baseContributionCapInWei,
        address tokenAddress
    ) public {
        require(alfaToEtherRate > 0);
        require(beneficiaryAddr != address(0));
        require(tokenAddress != address(0));

        token = AlfaToken(tokenAddress);
        rate = alfaToEtherRate;
    ...
    function updateRate(uint256 alfaToEtherRate) public onlyOwner atStage(Stages.Setup) {
        rate = alfaToEtherRate;
    }
     ...
     *
     * @param to Participant address to send alfatokens to
     * @param tokens Amount of alfatokens to be sent to parcitipant 
     */
